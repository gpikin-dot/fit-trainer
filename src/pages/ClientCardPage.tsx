...
export default function ClientCardPage() {
  ...
  if (!client) return (
    <Layout>
      <div className="text-center py-12 text-[var(--slate-400)] text-[var(--fs-xs)]">Клиент не найден</div>
    </Layout>
  )
  ...
  return (
    <Layout>
      ...
      <div className="sticky top-0 z-10 bg-white -mx-[13px] px-[13px]">
        <div className="pt-[11px] pb-[10px]">
          ...
          <div className="flex items-center gap-[8px] mb-[10px]">
            <div className="w-[32px] h-[32px] rounded-full bg-[var(--indigo-50)] border-[1.5px] border-[var(--indigo-200)] flex items-center justify-center shrink-0 text-[var(--fs-md)] font-bold text-[var(--indigo-500)]">
              {client.name.charAt(0).toUpperCase()}
            </div>
            <div>
              <div className="text-[var(--fs-md)] font-bold text-[var(--slate-900)] truncate">{client.name}</div>
          ...
          <button
            ...
            className="w-full border-[1.5px] border-dashed border-[var(--indigo-300)] bg-[var(--indigo-50)] rounded-[9px] py-[9px] text-[var(--fs-2xs)] font-bold text-[var(--indigo-500)] flex items-center justify-center gap-1 mb-[10px]"
          >
      ...
      <div className="px-0 pt-[11px] pb-[14px]">
        ...
        {tab === 'active' && (
          active.length === 0
            ? <div className="text-center text-[var(--fs-xs)] text-[var(--slate-400)] leading-[1.6] py-[28px]">Нет активных тренировок</div>
            : active.map(a => {
        ...
        {tab === 'history' && (
          history.length === 0
            ? <div className="text-center text-[var(--fs-xs)] text-[var(--slate-400)] leading-[1.6] py-[28px]">История пуста</div>
            : history.map(a => {
                const done = a.results.filter(r => r.completed).length
                const totalEx = a.exercises.length
