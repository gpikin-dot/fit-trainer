...
export default function ClientCardPage() {
  ...
  if (!client) return (
    <Layout>
      <div className="text-center py-12 text-[var(--slate-400)] text-[var(--text-sub)]">Клиент не найден</div>
    </Layout>
  )
  ...
  return (
    <Layout>
      ...
      <div className="sticky top-0 z-10 bg-white -mx-[13px] px-[13px]">
        <div className="pt-[11px] pb-[10px]">
          ...
          <button
            ...
            className="w-full border-[1.5px] border-dashed border-[var(--indigo-300)] bg-[var(--indigo-50)] rounded-[9px] py-[9px] text-[var(--text-sub)] font-bold text-[var(--indigo-500)] flex items-center justify-center gap-1 mb-[10px]"
          >
      ...
      <div className="px-0 pt-[11px] pb-[14px]">
        ...
        {tab === 'active' && (
          active.length === 0
            ? <div className="text-center text-[var(--text-sub)] text-[var(--slate-400)] leading-[1.6] py-[28px]">Нет активных тренировок</div>
            : active.map(a => {
                  ...
                  <div key={a.id} className="bg-white border border-[var(--border)] rounded-[10px] px-[11px] py-[9px] mb-[5px]">
                    <div className="flex justify-between gap-[6px]">
                      <div className="flex-1 min-w-0">
                        <div className="text-[var(--text-sub)] font-semibold text-[var(--slate-900)]">{a.workout?.name ?? '—'}</div>
                        <div className="text-[var(--text-label)] text-[var(--slate-400)] mt-[2px]">{dateLabel}</div>
                      </div>
                      <span className={`text-[var(--text-label)] font-bold px-[7px] py-[2px] rounded-[20px] shrink-0 ${
        ...
        {tab === 'history' && (
          history.length === 0
            ? <div className="text-center text-[var(--text-sub)] text-[var(--slate-400)] leading-[1.6] py-[28px]">История пуста</div>
            : history.map(a => {
                const done = a.results.filter(r => r.completed).length
                const totalEx = a.exercises.length
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-[5px] shrink-0 items-center">
                      <span className={`text-[var(--text-label)] font-bold px-[7px] py-[2px] rounded-[20px] ${
