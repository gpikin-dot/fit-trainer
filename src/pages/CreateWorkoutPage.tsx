...
export default function CreateWorkoutPage() {
  ...
  const noteInput = 'bg-[var(--slate-50)] border border-[var(--slate-200)] rounded-[6px] px-[7px] py-[5px] text-[17px] text-[var(--slate-600)] italic w-full outline-none focus:border-indigo-400 text-left'
    ...
    <Layout>
      <div className="pt-[11px] pb-[14px]">
        ...
        <div className="mb-[10px]">
          ...
          <input
            ...
            className="w-full border border-[var(--slate-200)] rounded-[7px] px-[9px] py-[7px] text-[17px] text-[var(--slate-900)] bg-[var(--slate-50)] outline-none focus:border-indigo-400"
          />
        </div>
              ...
              {exType === 'cardio_time' ? (
                <>
                  <div className="grid grid-cols-3 gap-[4px] mb-[4px]">
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Интервалы</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Длит. (мин)</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Дистанция (км)</label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '4px' }}>
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Отдых (сек)</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Комментарий</label>
                  ...
                  <div className="mt-[4px]">
                    <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Целевой пульс (уд/мин)</label>
                ...
                <>
                  <div className="grid grid-cols-3 gap-[4px] mb-[4px]">
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Подходы</label>
                      <input type="text" inputMode="numeric" value={isNaN(ex.sets) ? '' : ex.sets}
                        onChange={e => updateExercise(ex.tempId, { sets: parseInt(e.target.value) })}
                        onBlur={() => { if (!ex.sets || ex.sets < 1) updateExercise(ex.tempId, { sets: 1 }) }}
                        onFocus={e => e.target.select()} className={numInput} />
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Повторения</label>
                      <input type="text" inputMode="numeric" value={isNaN(ex.reps) ? '' : ex.reps}
                        onChange={e => updateExercise(ex.tempId, { reps: parseInt(e.target.value) })}
                        onBlur={() => { if (!ex.reps || ex.reps < 1) updateExercise(ex.tempId, { reps: 1 }) }}
                        onFocus={e => e.target.select()} className={numInput} />
                ...
                <>
                  <div className="grid grid-cols-3 gap-[4px] mb-[4px]">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Повторы</label>
                      <input type="text" inputMode="numeric" value={isNaN(ex.reps) ? '' : ex.reps}
                        onChange={e => updateExercise(ex.tempId, { reps: parseInt(e.target.value) })}
                        onBlur={() => { if (!ex.reps || ex.reps < 1) updateExercise(ex.tempId, { reps: 1 }) }}
                        onFocus={e => e.target.select()} className={numInput} />
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Вес, кг</label>
                      <input type="text" inputMode="decimal" value={ex.weight_kg}
                        onChange={e => updateExercise(ex.tempId, { weight_kg: e.target.value })}
                        onFocus={e => e.target.select()} className={numInput} />
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '4px' }}>
                    <div>
                      <label className="block text-[11px] font-bold text-[var(--slate-400)] uppercase tracking-[0.04em] mb-[3px]">Отдых</label>
                      <input type="text" inputMode="numeric" value={ex.rest_sec ?? ''}
                        onChange={e => updateExercise(ex.tempId, { rest_sec: e.target.value ? parseInt(e.target.value) : null })}
                        onFocus={e => e.target.select()} placeholder="—" className={numInput} />
                    </div>
        ...
        <button
          ...
          className="border-[1.5px] border-dashed border-[var(--indigo-200)] bg-white rounded-[8px] py-[8px] text-[16px] font-bold text-[var(--indigo-500)] w-full flex items-center justify-center gap-1 mt-[6px] mb-[8px]"
        >
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="w-full bg-[var(--indigo-500)] hover:bg-[var(--indigo-700)] disabled:opacity-50 text-white text-[16px] font-bold rounded-[9px] py-[10px]"
        >
      ...
      {showLibraryModal && (
        <Modal onClose={() => { setShowLibraryModal(false); setSelectedLibraryIds(new Set()) }}>
          <p className="text-[17px] font-bold text-[var(--slate-900)] mb-[4px]">Выберите упражнения</p>
          <p className="text-[15px] text-[var(--slate-500)] mb-[12px]">Выбрано: {selectedLibraryIds.size}</p>
          <input
            value={librarySearch}
            onChange={e => setLibrarySearch(e.target.value)}
            placeholder="🔍 Поиск..."
            className="w-full border border-[var(--slate-200)] rounded-[8px] px-[9px] py-[7px] text-[17px] bg-[var(--slate-50)] outline-none focus:border-[var(--indigo-300)] mb-[9px]"
          />
          <div className="flex gap-[4px] flex-wrap mb-[9px]">
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setLibraryCategory(cat)}
                className={`text-[15px] font-semibold px-[8px] py-[3px] rounded-[20px] transition-colors ${
                  libraryCategory === cat
            ...
            {filteredLibrary.map(lib => {
              ...
              return (
                <button key={lib.id} onClick={() => toggleLibrarySelect(lib.id)}
                  ...
                  <div>
                    <div className="text-[17px] font-semibold text-[var(--slate-900)]">{lib.name_ru}</div>
                    <div className="text-[15px] text-[var(--slate-400)]">{lib.category}</div>
                  </div>
                </button>
              )
            })}
            {filteredLibrary.length === 0 && <p className="text-[15px] text-[var(--slate-400)] text-center py-4">Ничего не найдено</p>}
          </div>
          <button
            onClick={addSelectedExercises}
            disabled={selectedLibraryIds.size === 0}
            className="w-full bg-[var(--indigo-500)] hover:bg-[var(--indigo-700)] disabled:opacity-40 text-white text-[16px] font-bold py-[9px] rounded-[8px] transition-colors"
          >
