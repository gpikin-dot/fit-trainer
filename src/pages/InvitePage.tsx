  ...
  if (loading) return (
    <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center px-4">
      <div className="max-w-[390px] text-center">
        <div className="text-[var(--text-meta)] text-[var(--slate-400)]">Проверка...</div>
      </div>
    </div>
  )
    ...
    return (
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center px-4">
        <div className="max-w-[390px] text-center">
          ...
          <div className="text-[var(--text-meta)] font-semibold text-[var(--red-600)] leading-[1.4] mb-2">{message}</div>
          <div className="text-[var(--text-meta)] text-[var(--slate-400)] mb-4">
            Ссылка действует 7 дней и используется один раз
          </div>
          <button
            onClick={() => navigate('/login')}
            className="text-[var(--text-nav)] font-semibold text-[var(--indigo-500)] cursor-pointer"
          >
            ← На главную
          </button>
        </div>
