...
export default function SessionDetailPage() {
  ...
  if (notFound || !assignment || !workout) {
    return (
      <Layout>
        <div className="flex items-center justify-center py-20 text-[var(--slate-400)] text-[var(--text-sub)]">Не найдено</div>
      </Layout>
    )
  }
    ...
    <Layout>
      <div className="pt-[11px] pb-[80px]">
        ...
        <button
          ...
          className="text-[var(--text-sub)] font-semibold text-[var(--slate-500)] flex items-center gap-1 mb-[9px]"
        >
          ← {clientName || 'Клиент'}
        </button>

        <h1 className="text-[var(--text-heading)] font-bold text-[var(--slate-900)] tracking-[-0.01em]">{workout.name}</h1>
        <p className="text-[var(--text-label)] text-[var(--slate-400)] mb-[11px]">{fmtDate(completedAt)}</p>
        ...
        <div className="bg-white border border-[var(--border)] rounded-[10px] px-[12px] py-[11px] mb-[10px]">
          <div className="flex justify-between mb-[9px]">
            ...
            <div className="text-right">
              ...
              <div className="text-[var(--text-label)] text-[var(--slate-400)] mt-[2px]">упражнений</div>
            </div>
          </div>
        ...
        {exercises.map((ex) => {
          ...
          return (
            <div key={ex.id} className="bg-white border border-[var(--border)] rounded-[10px] px-[11px] py-[9px] mb-[5px]">
              <p className="text-[var(--text-sub)] font-bold text-[var(--slate-900)] mb-[4px]">{ex.name}</p>

              {hasResult ? (
                <>
                  <p className="text-[var(--text-label)] text-[var(--slate-400)] mb-[2px]">
                    план: {ex.sets}×{ex.reps} · {ex.weight_kg} кг
                  </p>
                  <p className="text-[var(--text-label)] text-[var(--slate-700)]">
                ...
                <p className="text-[var(--text-label)] text-[var(--slate-300)] italic">Пропущено</p>
              )}

              {result?.client_note ? (
                <p className="text-[var(--text-label)] text-[var(--slate-500)] italic mt-[3px]">«{result.client_note}»</p>
              ) : null}
            </div>
          )
        })}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[var(--border)] px-[13px] pt-[11px] pb-[16px] max-w-[390px] mx-auto">
        <button
          ...
          className="w-full bg-[var(--indigo-500)] hover:bg-[var(--indigo-700)] text-white rounded-[9px] py-[10px] text-[var(--text-sub)] font-bold mb-[6px]"
        >
          Повторить тренировку
        </button>
        <button
          onClick={() => navigate(`/trainer/workout/${workout.id}/edit`)}
          className="w-full bg-white border border-[var(--slate-200)] text-[var(--slate-700)] rounded-[9px] py-[9px] text-[var(--text-sub)] font-semibold"
        >
          Изменить тренировку
        </button>
      </div>
