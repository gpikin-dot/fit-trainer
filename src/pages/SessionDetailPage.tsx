...
export default function SessionDetailPage() {
  ...
  if (notFound || !assignment || !workout) {
    return (
      <Layout>
        <div className="flex items-center justify-center py-20 text-[var(--slate-400)] text-[var(--fs-xs)]">Не найдено</div>
      </Layout>
    )
  }
    ...
    <Layout>
        ...
        <div className="bg-white border border-[var(--border)] rounded-[10px] px-[12px] py-[11px] mb-[10px]">
          <div className="flex justify-between mb-[9px]">
            <div className="flex gap-[5px] items-center">
              ...
              <span className="text-[var(--fs-md)] font-bold text-[var(--green-600)]">Выполнена</span>
            </div>
        ...
        {exercises.map((ex) => {
          ...
          return (
            <div key={ex.id} className="bg-white border border-[var(--border)] rounded-[10px] px-[11px] py-[9px] mb-[5px]">
              <p className="text-[var(--fs-xs)] font-bold text-[var(--slate-900)] mb-[4px]">{ex.name}</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[var(--border)] px-[13px] pt-[11px] pb-[16px] max-w-[390px] mx-auto">
        <button
          ...
          className="w-full bg-[var(--indigo-500)] hover:bg-[var(--indigo-700)] text-white rounded-[9px] py-[10px] text-[var(--fs-xs)] font-bold mb-[6px]"
        >
          Повторить тренировку
        </button>
        <button
          onClick={() => navigate(`/trainer/workout/${workout.id}/edit`)}
          className="w-full bg-white border border-[var(--slate-200)] text-[var(--slate-700)] rounded-[9px] py-[9px] text-[var(--fs-xs)] font-semibold"
        >
          Изменить тренировку
        </button>
      </div>
