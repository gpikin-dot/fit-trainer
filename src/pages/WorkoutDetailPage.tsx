    ...
    <Layout>
      <div className="pt-[11px] pb-[14px]">
        ...
        <button
          ...
          className="w-full bg-[var(--indigo-500)] hover:bg-[var(--indigo-700)] text-white text-[var(--fs-xs)] font-bold rounded-[9px] py-[10px] mb-[15px]"
        >
          Назначить клиенту
        </button>
        ...
        {exercises.length === 0 ? (
          <p className="text-[var(--fs-xs)] text-[var(--slate-400)]">Нет упражнений</p>
        ) : (
          exercises.map((ex, i) => (
          ...
          <div className="mt-[13px]">
            ...
            <div className="bg-white border border-[var(--border)] rounded-[10px] overflow-hidden">
              {Array.from(clientUsage.values()).map(({ name, clientId, count }, idx, arr) => (
                ...
                >
                  <div className="w-[24px] h-[24px] rounded-full bg-[var(--indigo-50)] flex items-center justify-center shrink-0 text-[var(--fs-2xs)] font-bold text-[var(--indigo-500)]">
